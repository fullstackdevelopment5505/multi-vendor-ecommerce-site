      ...
      <div class="ps-section--shopping ps-shopping-cart">
        <div class="container">
          ...
          <div class="ps-section__content">
            <div class="table-responsive">
              <table class="table ps-table--shopping-cart">
                ...
                <tbody>
                   <?php $i = 1; ?>
                   <?php foreach ($this->cart->contents() as $items): ?>   
                  <tr id='table_<?=$items['rowid']?>'>
                    <td>
                      <div class="ps-product--cart" >
                        <div class="ps-product__thumbnail"><a href="product-default.html"><img src="<?php echo base_url().'assets/app-assets/images/product/'.$items['img']?>" alt="" /></a></div>
                        <div class="ps-product__content"><a href="product-default.html"><?php echo $items['name']; ?></a>
                    ...
                    <td>
                      <div class="form-group--number">
                        <button ng-click="updownBtn('<?=$items['rowid']?>','<?=$items['qty']?>','1')"class="up">+</button>
                        <button ng-click="updownBtn('<?=$items['rowid']?>','<?=$items['qty']?>','-1')"class="down">-</button>
                        <input id="<?=$items['rowid']?>_qty"class="form-control" type="text" placeholder="1" value='<?php echo $items['qty']; ?>'>
                      </div>
                    </td>
                    <td>$<?php echo $this->cart->format_number($items['subtotal']); ?></td>
                    <td><a ng-click="crossBtn('<?=$items['rowid']?>','<?=$items['subtotal']?>','<?=$items['qty']?>')"><i class="icon-cross"></i></a></td>
                  </tr>
                  <?php $i++; ?>
                  <?php endforeach; ?>
                </tbody>
            ...
            <div class="ps-section__cart-actions"><a class="ps-btn" href="<?php echo base_url().'index.php/home/shop'?>"><i class="icon-arrow-left"></i> Back to Shop</a>
            <?php if($this->session->has_userdata('customerId')){  ?>
              <a class="ps-btn ps-btn--outline" href="<?php echo base_url()?>index.php/paypal/buy"><i class="icon-sync"></i> Submit Inquiry</a></div>
            <?php }else{ ?>
              <a class="ps-btn ps-btn--outline" href="<?php echo base_url()?>index.php/login">Login/Register</a></div>
            <?php } ?>
          </div>
        </div>
      </div>
    ...
    App.controller('homeController', function($scope, $http){ 
      ...
      $scope.totalItems = Number.parseInt('<?=$this->cart->total_items();?>');
        $scope.crossBtn = function(rowId,sub, amount){
              // $scope.flag = false;
                ...
                $http.post(base_url + 'index.php/delcart',JSON.stringify(data)).then(function (response){
                    ...
                    {     
                        ...
                        location.assign('<?php echo base_url()."index.php/home/cart"?>');
                        // alert('true');
                    }
                    else{
                        alert('false');
            ...
            $scope.updownBtn = function(aid, aqty, aflag){
                temp = '#' + aid + '_qty';
                qty = Number.parseInt($(temp).val());
                // if(qty <= 1 && aflag == '-1') break;
                data = {
                    id: aid,
                    qty: aqty,
                    flag: aflag,
                };
                $http.post(base_url + 'index.php/updatecart',JSON.stringify(data)).then(function (response){
                    // alert(JSON.stringify(response.data));
                    if(response.data == 'true')
                    {     
                      location.assign('<?php echo base_url()."index.php/home/cart"?>');
